...
if (!defined('TEMPLATE_LITE_DIR')) {
	define('TEMPLATE_LITE_DIR', dirname(__FILE__) . DIR_SEPARATOR);
}
class Template_Lite {
	...
	var $compile_dir		= "../../tmp/compiled";	// the directory to store the compiled files in
	var $config_dir			= "templates";	// where the config files are
	var $cache_dir			= "../../tmp/cached";	// where cache files are stored
	...
	{
		global $_config, $_cache, $PAGE_TITLE, $PAGE_ID, $_SESSION, $side_menu, $sub_menu, $_langs;
		$this->compile_dir = $_config['smarty_cache'] . '/compiled';
		$this->cache_dir = $_config['smarty_cache'] . '/cached';
		date_default_timezone_set('Europe/Sofia');
		$this->_version_date = strtotime($this->_version_date);
		$this->assign_by_ref('_config', $_config);
		$this->assign_by_ref('_session', $_SESSION);
		$this->assign_by_ref('_langs', $_langs);
		$this->assign_by_ref('GLOBALS', $GLOBALS);
		$this->assign_by_ref('PAGE_TITLE', $PAGE_TITLE);
		$this->assign_by_ref('PAGE_ID', $PAGE_ID);
		if($side_menu) $this->assign_by_ref('side_menu', $side_menu);
		if($sub_menu) $this->assign_by_ref('sub_menu', $sub_menu);
...
//		$this->_error_level = error_reporting(E_ALL);
	...
	function _get_plugin_dir($plugin_name)
	{
		...
		foreach ($_plugin_dir_list as $_plugin_dir)
		{
			...
			{
				// path is relative
				if (file_exists(dirname(__FILE__) . DIR_SEPARATOR . $_plugin_dir . DIR_SEPARATOR . $plugin_name))
				{
					$plugin_dir_path = dirname(__FILE__) . DIR_SEPARATOR . $_plugin_dir . DIR_SEPARATOR;
					break;
				}
			}
			else
			{
				...
				{
					if (file_exists($_include_path . DIR_SEPARATOR . $plugin_name))
					{
						$plugin_dir_path = $_include_path . DIR_SEPARATOR;
						break 2;
					}
				}
			}
