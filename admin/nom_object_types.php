...
switch ($FILTER['mode']) {
    case 'hide':
        $mm->Query("UPDATE type SET is_deleted=1 WHERE id={$FILTER['id']}");
        redirect($FILTER['back_url']);
        break;
    case 'unhide':
        $mm->Query("UPDATE type SET is_deleted=0 WHERE id={$FILTER['id']}");
        redirect($FILTER['back_url']);
        break;
    case 'edit':
        edit($FILTER);
...
function show(&$FILTER) {
    ...
    $tpl->display('nom_object_types.tpl');
}
...
function edit(&$FILTER,$errs = array()){
    ...
    $tpl->assign_by_ref('FILTER', $FILTER);
    $tpl->assign_by_ref('errs', $errs);
    $tpl->display('nom_object_types.tpl', $cache);
}
...
function insertUpdate(&$FILTER) {
    ...
    if($FILTER['id']>0) {
        $res = $mm->AutoExecute('type', $FILTER['item'], 2, "id={$FILTER['id']}", true);
    } else {
        $res = $mm->AutoExecute('type', $FILTER['item'], 1, false);
    }
